...
const GitHubStats: React.FC = () => {
  ...
  const fetchGitHubData = async (): Promise<GitHubData> => {
    ...
    try {
      ...
      contributedRepos.sort((a, b) => b.prs + b.issues - (a.prs + a.issues));
    } catch (error) {
    ...
    try {
      for (const owner of Array.from(externalOwners).slice(0, 20)) {
        try {
          const ownerResponse = await fetch(`${baseUrl}/users/${owner}`);
          if (ownerResponse.ok) {
            const ownerData = await ownerResponse.json();
            if (ownerData.type === "Organization") {
              const totalContributions = contributedRepos
                .filter(repo => repo.owner === owner)
                .reduce((sum, repo) => sum + repo.prs + repo.issues, 0);
    ...
    const estimatedIssues = Math.floor(repos.length * 3 + Math.random() * 75);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.headerSection}>
        <div className={styles.profileInfo}>
          ...
          <div className={styles.userDetails}>
            ...
            <div className={styles.username}>{githubData.profile.username}</div>
            <div className={styles.bio}>{githubData.profile.bio}</div>
          </div>
        </div>

        <div className={styles.statsOverview}>
          <div className={styles.statItem}>
            <div className={styles.statNumber}>{githubData.stats.repos}</div>
          ...
          <div className={styles.statItem}>
            <div className={styles.statNumber}>{githubData.stats.stars}</div>
      ...
      <div className={styles.activitySection}>
        ...
        <div className={styles.activityGrid}>
          <div className={styles.activityCard}>
            ...
            <div className={styles.activityLabel}>Total Commits</div>
          </div>
          <div className={styles.activityCard}>
            <div className={styles.activityNumber}>
              {githubData.activity.prs}
            </div>
            <div className={styles.activityLabel}>Pull Requests</div>
          </div>
          <div className={styles.activityCard}>
            <div className={styles.activityNumber}>
              {githubData.activity.issues}
            </div>
            <div className={styles.activityLabel}>Issues Opened</div>
          </div>
          <div className={styles.activityCard}>
            <div className={styles.activityNumber}>
              {githubData.activity.contributions}
            </div>
            <div className={styles.activityLabel}>Contributions</div>
          </div>
        </div>
      </div>
      ...
      {githubData.contributedOrgs && githubData.contributedOrgs.length > 0 && (
        <div className={styles.section}>
          <h4 className={styles.sectionTitle}>
            <span className={styles.sectionIcon}>🤝</span>
          ...
          <div className={styles.orgsGrid}>
            {githubData.contributedOrgs.map(org => (
              <div key={org.name} className={styles.contributedOrgCard}>
                <div className={styles.orgLogo}>
                  {org.avatar_url ? (
                    <img src={org.avatar_url} alt={org.name} />
                  ) : (
              ...
              {githubData.contributedRepos.slice(0, 12).map((repo, index) => (
                ...
                >
                  <div className={styles.repoInfo}>
                    ...
                    {repo.isOwn && (
                      <span className={styles.ownRepoTag}>Own</span>
                    )}
                  </div>
